...
@section('titulo', 'Editar Comprador')
@section('page-title', 'Editar Comprador')
@section('page-subtitle', $comprador->name)

@section('header-actions')
<a href="{{ route('admin.compradores.show', $comprador->id) }}" class="flex items-center gap-2 px-4 py-2 text-[var(--cor-texto-secundario)] hover:text-[var(--cor-verde-serra)] transition-colors">
...
<div class="p-4 lg:p-8">
    <div class="max-w-4xl mx-auto">
        ...
        <form method="POST" action="{{ route('admin.compradores.update', $comprador->id) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-xl shadow-sm border border-[var(--cor-borda)] p-6">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $comprador->name) }}" required
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email', $comprador->email) }}" required
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--cor-verde-serra)] focus:border-transparent">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="plano" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--cor-verde-serra)]">
                            <option value="gratis" {{ old('plano', $comprador->plano) === 'gratis' ? 'selected' : '' }}>Grátis</option>
                            <option value="premium" {{ old('plano', $comprador->plano) === 'premium' ? 'selected' : '' }}>Premium</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-[var(--cor-borda)] p-6">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="nome_negocio" value="{{ old('nome_negocio', $comprador->comprador?->nome_negocio) }}" required
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--cor-verde-serra)] focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-[var(--cor-texto)] mb-2">CNPJ</label>
                        <input type="text" name="cnpj" value="{{ old('cnpj', $comprador->comprador?->cnpj) }}"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--cor-verde-serra)] focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-[var(--cor-texto)] mb-2">Tipo de Negócio</label>
                        <input type="text" name="tipo_negocio" value="{{ old('tipo_negocio', $comprador->comprador?->tipo_negocio) }}"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--cor-verde-serra)] focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-[var(--cor-texto)] mb-2">Colaboradores</label>
                        <input type="number" name="colaboradores" value="{{ old('colaboradores', $comprador->comprador?->colaboradores) }}"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--cor-verde-serra)] focus:border-transparent">
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-[var(--cor-texto)] mb-2">Site</label>
                        <input type="url" name="site_url" value="{{ old('site_url', $comprador->comprador?->site_url) }}"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--cor-verde-serra)] focus:border-transparent">
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-[var(--cor-texto)] mb-2">Descrição</label>
                        <textarea name="descricao" rows="4"
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--cor-verde-serra)] focus:border-transparent">{{ old('descricao', $comprador->comprador?->descricao) }}</textarea>
                    </div>
                    <div class="md:col-span-2">
