...
<div class="p-4 lg:p-8 max-w-4xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900 mt-2">Editar Material</h1>
    </div>

    <form action="{{ route('admin.materiais.update', $material->id) }}" method="POST" enctype="multipart/form-data" 
          x-data="{ tipo: '{{ old('tipo', $material->tipo) }}', previewThumbnail: null }" class="bg-white rounded-lg shadow-sm p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6">
            ...
            <textarea name="descricao" rows="4" 
                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">{{ old('descricao', $material->descricao) }}</textarea>
            ...
            <div>
                ...
                <select name="categoria" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                    ...
                    @foreach($categorias as $key => $label)
                        <option value="{{ $key }}" {{ old('categoria', $material->categoria) === $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
        ...
        <div x-show="tipo === 'arquivo'" class="mb-6">
            ...
            @if($material->arquivo_path)
                <p class="text-sm text-gray-600 mb-2">
                    ...
                    Arquivo atual: <a href="{{ asset('storage/' . $material->arquivo_path) }}" target="_blank" class="text-primary hover:underline">{{ basename($material->arquivo_path) }}</a>
                </p>
            @endif
            <input type="file" name="arquivo" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx" 
                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
            <p class="text-xs text-gray-500 mt-1">PDF, DOC, DOCX, XLS, XLSX, PPT, PPTX (máx. 10MB) - Deixe em branco para manter o atual</p>
        ...
        <div x-show="tipo === 'video'" class="mb-6">
            ...
            <input type="url" name="video_url" value="{{ old('video_url', $material->video_url) }}" placeholder="https://www.youtube.com/watch?v=..." 
                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
        ...
        <div x-show="tipo === 'link'" class="mb-6">
            ...
            <input type="url" name="link_externo" value="{{ old('link_externo', $material->link_externo) }}" placeholder="https://..." 
                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
        ...
        <div class="mb-6">
            ...
            @if($material->thumbnail_url)
                <div class="mb-3">
                    <p class="text-sm text-gray-600 mb-2">Thumbnail atual:</p>
                    <img src="{{ asset('storage/' . $material->thumbnail_url) }}" class="w-full max-w-md h-48 object-cover rounded-lg">
                </div>
            @endif
            ...
            <input type="file" name="thumbnail" accept="image/jpeg,image/jpg,image/png,image/webp" 
                   ...
                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
            <p class="text-xs text-gray-500 mt-1">JPG, PNG, WEBP (máx. 2MB, recomendado 1280x720) - Deixe em branco para manter a atual</p>
            
            <!-- Preview -->
            <div x-show="previewThumbnail" class="mt-3">
                <p class="text-sm text-gray-600 mb-2">Nova thumbnail:</p>
        ...
        <div class="bg-blue-50 rounded-lg p-4 mb-6">
            <h3 class="font-bold text-gray-900 mb-2">Estatísticas</h3>
            <div class="grid grid-cols-3 gap-4 text-center">
                ...
                <div>
                    <p class="text-2xl font-bold text-primary">{{ $material->created_at->format('d/m/Y') }}</p>
