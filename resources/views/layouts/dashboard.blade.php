...
<body class="min-h-screen bg-[var(--cor-fundo)] text-[var(--cor-texto)] font-sans antialiased overflow-x-hidden">
    ...
    <nav class="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-[var(--cor-borda)] z-50 safe-area-pb shadow-[0_-4px_20px_rgba(0,0,0,0.08)]" 
        ...
        <div x-show="menuAberto"
            ...
            {{-- Header do Menu --}}
            <div class="sticky top-0 bg-gradient-to-br from-[var(--cor-verde-serra)] to-green-700 text-white p-6 flex items-center justify-between shadow-md z-10">
            ...
            {{-- Footer do Menu --}}
            <div class="sticky bottom-0 bg-gradient-to-t from-gray-100 to-gray-50 border-t border-[var(--cor-borda)] p-4 shadow-[0_-4px_12px_rgba(0,0,0,0.05)]">
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-lg hover:from-red-700 hover:to-red-800 transition-all font-medium shadow-lg">
                        <i data-lucide="log-out" class="w-4 h-4"></i>
                        Sair da Conta
                    </button>
                </form>
